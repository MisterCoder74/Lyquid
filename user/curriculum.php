...
<?php
        ...
        // Recuperiamo i dati di performance dell'utente loggato
        $user_performance = json_decode(file_get_contents("../dbs/user_performance.json"), true);
        $performance_data = array();
        foreach ($user_performance as $user_perf) {
            if ($user_perf['Surname'] == $current_user['Surname']) {
                $performance_data[$user_perf['performances'][0]['project_name']] = $user_perf['performances'][0]['performance_value'];
            }
        }
        ...
        echo "</ul>";
// Mostrare i progetti su cui ha lavorato e le relative performance
...
foreach ($user_projects as $project) {
    ...
    // Mostrare la performance del progetto per l'utente loggato
$performance_list = json_decode(file_get_contents("../dbs/user_performance.json"), true);
// Mostrare la performance del progetto per l'utente loggato
$performance_list = json_decode(file_get_contents("../dbs/user_performance.json"), true);
foreach ($performance_list as $performance) {
    if ($performance['Surname'] == $current_user['Surname']) {
        foreach ($performance['performances'] as $performance_item) {
            if ($performance_item['project_name'] == $project['name']) {
                echo "<li><strong>Performance:</strong> " . str_replace('_', ' ', $performance_item['performance_value']) . "</li>";
